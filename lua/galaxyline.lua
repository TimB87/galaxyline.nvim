    ...
    ['string'] = function()
      if galaxyline_providers[provider] == nil then
        if next(galaxyline_providers) ~= nil then
          print(
            string.format(
              'provider of %s does not exist in default provider group',
              component_name
            )
          )
          return ''
    ...
    if condition() then
      ...
      if separator then
        if position == 'left' then
          tmp_line = tmp_line
            .. generate_separator_section(component, separator)
        else
...
local function load_section(section_area, pos)
  ...
  for _, component in pairs(section_area) do
    for component_name, component_info in pairs(component) do
      local ls =
        section_complete_with_option(component_name, component_info, pos)
      section = section .. ls
      ...
      if
        ...
        and vim.fn.index(events, component_info.event) == -1
      then
...
function galaxyline.load_galaxyline()
  if
    vim.o.laststatus ~= 3 and vim.api.nvim_win_get_config(0).relative ~= ''
  then
    return
  end
  ...
  local mid_section = next(galaxyline.section.mid) ~= nil
      and load_section(galaxyline.section.mid, 'mid')
    or nil
  local short_left_section =
    load_section(galaxyline.section.short_line_left, 'left')
  local short_mid_section = next(galaxyline.section.short_line_mid) ~= nil
      and load_section(galaxyline.section.short_line_mid, 'mid')
    or nil
  local short_right_section =
    load_section(galaxyline.section.short_line_right, 'right')
  local line
...
function galaxyline.galaxyline_augroup()
  ...
  for _, def in ipairs(events) do
    local command = string.format(
      'autocmd %s * lua require("galaxyline").load_galaxyline()',
      def
    )
